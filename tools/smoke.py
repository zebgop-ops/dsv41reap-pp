#!/usr/bin/env python3
"""Quick correctness + speed check against the DSv41Flash server (no deps beyond stdlib)."""
import json, sys, time, urllib.request
URL = sys.argv[1] if len(sys.argv) > 1 else "http://localhost:8004"
def chat(msgs, max_tokens=256, effort=None, stream=False, **kw):
    body = {"model": "DSv41Flash", "messages": msgs, "max_tokens": max_tokens, "temperature": 0.0, **kw}
...
m, u, dt = chat([{"role": "user", "content": "Write a 150-word paragraph about the history of the bicycle."}], max_tokens=800, effort="low")
print(f"\nparagraph: {(m.get('content') or '')[:300]!r}... reasoning_len={len(m.get('reasoning') or '')}\nusage: {u}  decode {u['completion_tokens']/dt:.1f} tok/s over {dt:.1f}s")
